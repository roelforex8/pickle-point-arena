...
import { nextDateInputValue } from './dateInput';
...
const statusDetails = {
  ...
  walkIn: { label: 'Confirmed Walk-In booking', short: 'WALK-IN' },
...
function philippineGreeting(date = new Date()) {
  ...
  return (
    <main>
      ...
      <section className="hero">
        <div className="hero-copy">
          ...
          <div className="hero-facts">
            ...
            <div><strong>6AM–12AM</strong><span>Open daily</span></div>
      ...
      <footer>
        ...
        <div><strong>Hours</strong><span>Monday–Sunday</span><span>6:00 AM–12:00 AM</span></div>
        <div><strong>Rates</strong><span>₱300 · 6AM - 4PM</span><span>₱350 · 4PM - 12AM</span></div>
...
function BookingCalendar({ selectedDate, setSelectedDate, selectedSlots, setSelectedSlots, onTrackBooking, browserInfo }) {
  ...
  const refreshAvailability = useCallback(async () => {
    ...
    const to = manilaSlotIso(selectedDate, 0);
  ...
  return (
    <div className="portal-shell calendar-portal">
      <div className="portal-heading">
        <div><span className="eyebrow dark">LIVE COURT AVAILABILITY</span><h2>Choose your game time.</h2><p>Open daily from 6:00 AM to 12:00 AM. Select consecutive hours or multiple courts.</p></div>
      </div>
      ...
      <div className="calendar-toolbar">
        ...
        <div className="rate-guide"><span><b>₱300</b> 6AM - 4PM</span><span><b>₱350</b> 4PM - 12AM</span></div>
      </div>
...
function AdminPortal() {
  ...
  if (passwordRecovery && staffRole && staffSession) {
    return <main className="admin-login-page password-reset-page"><header className="admin-login-header"><a href="/" className="brand brand-image" aria-label="Return to Pickle Point Arena public site"><img src="/header-logo-colored.png" alt="Pickle Point Arena" /></a><span>Secure account recovery</span></header><section className="password-reset-layout"><form className="admin-login-card password-reset-card" onSubmit={completePasswordRecovery}><span className="summary-tag">PASSWORD RECOVERY</span><h2>Choose a new password.</h2><p>The reset link was verified for {staffSession.user.email}. Enter a new password for this account.</p><label>New password<PasswordInput value={recoveryPassword} onChange={(event) => setRecoveryPassword(event.target.value)} autoComplete="new-password" minLength="7" required /></label><label>Confirm new password<PasswordInput value={recoveryPasswordConfirm} onChange={(event) => setRecoveryPasswordConfirm(event.target.value)} autoComplete="new-password" minLength="7" required /></label><small className="password-rules">{passwordRequirements}</small><button className="primary full" type="submit" disabled={recoverySubmitting}>{recoverySubmitting ? 'Updating securely…' : 'Reset password'} <span>→</span></button>{recoveryMessage && <p className="login-message">{recoveryMessage}</p>}</form></section></main>;
  }
  ...
  if (!staffRole) {
    return <main className="admin-login-page">
      ...
      <section className="admin-login-layout">
        ...
        <form className="admin-login-card" onSubmit={signIn}><span className="summary-tag">OWNER / ADMIN SIGN IN</span><h2>Welcome back.</h2><p>Use the email and password assigned by the Owner.</p><label>Email address<input type="email" value={loginEmail} onChange={(event) => setLoginEmail(event.target.value)} placeholder="you@email.com" autoComplete="username" required /></label><label>Password<PasswordInput value={loginPassword} onChange={(event) => setLoginPassword(event.target.value)} placeholder="Enter your password" autoComplete="current-password" required /></label><div className="login-options"><span>Secure Supabase authentication</span><button type="button" onClick={requestPasswordReset} disabled={forgotSubmitting}>{forgotSubmitting ? 'Sending…' : 'Forgot password?'}</button></div><button className="primary full" type="submit" disabled={loginSubmitting || forgotSubmitting}>{loginSubmitting ? 'Signing in…' : 'Sign in securely'} <span>→</span></button>{loginMessage && <p className="login-message">{loginMessage}</p>}<small className="login-disclaimer">Only active Owner and Administrator accounts can access this portal.</small></form>
      </section>
    </main>;
  }
...
function OperationsCalendar({ selectedDate, setSelectedDate, refreshKey, role, session, onChanged }) {
  ...
  const loadSchedule = useCallback(async ({ showLoading = false } = {}) => {
    ...
    const rangeEnd = manilaSlotIso(selectedDate, 0);
  ...
  const getSlotData = (hour, courtIndex) => {
    ...
    const mapped = { held: 'awaiting', payment_submitted: 'pending', confirmed: booking?.booking_source === 'walk_in' ? 'walkIn' : 'booked' };
    if (start <= nowTick) {
  ...
  const openWalkInReview = () => {
    ...
    setWalkInMessage('');
    setWalkInReviewOpen(true);
  };

  const confirmWalkInBooking = async () => {
    if (!walkInSelections.length || walkInSubmitting) return;
    setWalkInSubmitting(true);
    setWalkInMessage('');
    try {
      const response = await postStaffWalkIn(supabase, walkInSelections.map(({ date, hour, courtId }) => ({ date, hour, courtId })), walkInCustomerName.trim());
      const result = await response.json();
      if (!response.ok) return setWalkInMessage(result.error || 'The Walk-In booking could not be created.');
      setWalkInReviewOpen(false);
      setWalkInCustomerName('');
      setAvailabilitySelections(new Map());
      setCalendarMessage(`${result.booking?.trackingNumber || 'Walk-In'} confirmed · ₱${Number(result.booking?.totalAmount || 0).toLocaleString()} · Booking fee ₱0.`);
      await loadSchedule();
      onChanged?.();
    } catch (error) {
      setWalkInMessage(error.message || 'The Walk-In booking could not be created.');
    } finally {
  ...
  return <section className={`operations-calendar ${scheduleOpen ? 'expanded' : 'collapsed'}`}>
    ...
    {scheduleOpen && <div className="operations-calendar-content"><label className="operations-date-picker">Date<input type="date" value={selectedDate} onChange={(event) => setSelectedDate((currentDate) => nextDateInputValue(currentDate, event.target.value))} /></label><div className="week-strip operations-week">{weekDays.map((day) => <button key={dateKey(day)} className={dateKey(day) === selectedDate ? 'active' : ''} onClick={() => setSelectedDate(dateKey(day))}><small>{day.toLocaleDateString('en-US', { weekday: 'short' }).toUpperCase()}</small><strong>{day.getDate()}</strong></button>)}</div>
...
function PaymentReview({ session, refreshKey, onChanged, activityPanel }) {
  ...
  return <>
    ...
    <div className="dashboard-priority-row">
      {activityPanel}
      <section className="recent-activity-card"><div className="recent-activity-heading"><small>RECENT ACTIVITY · LAST 7 DAYS</small><h3>Payment Review History</h3><p>Undo is available for 30 minutes after each decision, whether used or not.</p></div><div className="decision-history recent-decision-list">{recentActivities.length === 0 ? <p className="recent-activity-empty">No payment decisions in the last seven days.</p> : recentActivities.map((booking) => {
        const payment = Array.isArray(booking.payments) ? booking.payments[0] : booking.payments;
        const reviewedAt = new Date(payment?.reviewed_at || 0).getTime();
...
function OwnerSalesReport({ session, refreshKey }) {
  ...
  const loadReport = useCallback(async () => {
    ...
    const response = await fetch(`/api/reports?period=${period}&source=${reportSource}${rangeQuery}`, { headers: { Authorization: `Bearer ${session.access_token}` } });
    const result = await response.json();
    if (response.ok) {
      setReport(result);
      setMessage('Confirmed bookings only · live owner report.');
    } else setMessage(result.error || 'The sales report could not be loaded.');
  }, [period, reportSource, appliedRange, session?.access_token]);
  ...
  return <section className="owner-sales-report">
    <div className="sales-report-heading"><div><small>OWNER FINANCIAL REPORT</small><h3>Confirmed sales comparison</h3><p>{message}</p></div><div className="sales-heading-actions"><div className="period-toggle source-toggle" aria-label="Booking source">{[{ value: 'all', label: 'All' }, { value: 'online', label: 'Online' }, { value: 'walk_in', label: 'Walk-In' }].map(({ value, label }) => <button type="button" className={reportSource === value ? 'active' : ''} onClick={() => setReportSource(value)} key={value}>{label}</button>)}</div><div className="period-toggle" aria-label="Revenue comparison period">{['day', 'week', 'month'].map((value) => <button type="button" className={period === value ? 'active' : ''} onClick={() => setPeriod(value)} key={value}>{value[0].toUpperCase() + value.slice(1)}</button>)}</div><button type="button" onClick={loadReport}>Refresh report</button></div></div>
    ...
    {reportSource !== 'online' && <section className="walk-in-report"><div><small>WALK-IN BOOKINGS · {currentLabel}</small><h3>Confirmed Walk-In activity</h3></div>{report?.walkInBookings?.length ? <div className="walk-in-report-list">{report.walkInBookings.map((booking) => <article key={booking.id}><div><strong>{booking.trackingNumber}</strong><span>{new Date(booking.confirmedAt).toLocaleString('en-US', { timeZone: 'Asia/Manila', month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' })}</span></div><div><strong>{booking.slots.map((slot) => `Court ${slot.courtId}`).join(', ')}</strong><span>{booking.slots.map((slot) => `${new Date(slot.slotStart).toLocaleDateString('en-US', { timeZone: 'Asia/Manila', month: 'short', day: 'numeric' })} · ${new Date(slot.slotStart).toLocaleTimeString('en-US', { timeZone: 'Asia/Manila', hour: 'numeric', minute: '2-digit' })}`).join(' · ')}</span></div><div><span>Created by {booking.createdBy}</span><strong>₱{Number(booking.amount || 0).toLocaleString()}</strong></div></article>)}</div> : <p>No confirmed Walk-In bookings in this period.</p>}</section>}
...
function OwnerPreview({ role = 'owner', session, selectedDate, setSelectedDate }) {
  ...
  const [dashboardGreeting, setDashboardGreeting] = useState(() => philippineGreeting());
  ...
  useEffect(() => {
    const updateGreeting = () => setDashboardGreeting(philippineGreeting());
    updateGreeting();
    const timer = window.setInterval(updateGreeting, 60 * 1000);
    return () => window.clearInterval(timer);
  }, []);
  ...
  const changePassword = async (event) => {
    ...
    try {
      await changeOwnerPassword(supabase.auth, { currentPassword, newPassword });
      ...
      <section className="team-manager">
        <form className={`owner-account ${ownerAccountOpen ? 'expanded' : 'collapsed'}`} onSubmit={changePassword}>
          ...
          {ownerAccountOpen && <div className="team-panel-content">
            ...
            <label>Current password<PasswordInput value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} autoComplete="current-password" required /></label>
            <label>New password<PasswordInput value={newPassword} onChange={(event) => setNewPassword(event.target.value)} autoComplete="new-password" minLength="7" aria-describedby="owner-password-rules" required /></label>
            <label>Confirm new password<PasswordInput value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} autoComplete="new-password" minLength="7" required /></label>
            ...
            <div className="owner-pin-settings"><div><small>BOOKING CANCELLATION PIN</small><h4>Four-digit owner PIN</h4><span className={pinConfigured ? 'configured' : ''}>{pinConfigured ? 'PIN configured' : 'No PIN configured'}</span></div><label>New PIN<PasswordInput inputMode="numeric" pattern="[0-9]{4}" maxLength="4" value={cancellationPin} onChange={(event) => setCancellationPin(event.target.value.replace(/\D/g, '').slice(0, 4))} autoComplete="off" /></label><label>Confirm PIN<PasswordInput inputMode="numeric" pattern="[0-9]{4}" maxLength="4" value={confirmCancellationPin} onChange={(event) => setConfirmCancellationPin(event.target.value.replace(/\D/g, '').slice(0, 4))} autoComplete="off" /></label><button type="button" onClick={saveCancellationPin} disabled={pinSubmitting}>{pinSubmitting ? 'Saving…' : pinConfigured ? 'Change PIN' : 'Set PIN'}</button>{pinMessage && <p>{pinMessage}</p>}<small>The PIN is encrypted and cannot be displayed after saving.</small></div>
          </div>}
        </form>
